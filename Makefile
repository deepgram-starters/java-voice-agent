...
help:
	@echo "Java Voice Agent - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Setup:"
...
check-prereqs:
	...
	@command -v mvn >/dev/null 2>&1 || { echo "mvn is required but not installed. Visit https://maven.apache.org"; exit 1; }
...
start-backend:
	...
	@if [ ! -f "target/java-voice-agent-1.0.0-shaded.jar" ]; then \
		echo "Error: Shaded JAR not found. Run 'make install' first."; \
		exit 1; \
	fi
	@echo "==> Starting backend on http://localhost:8081"
	java -jar target/java-voice-agent-1.0.0-shaded.jar
